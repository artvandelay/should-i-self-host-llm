...
| 21 | Inference throughput formula | `960 tok/s per 8B active params per H100-equivalent unit`, scaled linearly by `bf16_tflops / 989` so a B200 (2250 TFLOPS) gets ~2.3× the throughput automatically. Still a heuristic — real throughput depends on context length, batch, KV-cache reuse, attention impl. Calibrated for ~4–8K context, 1–4 concurrent requests. | Industry rule-of-thumb; vLLM / TGI benchmarks at typical config | MED | N — known approximation, documented. |
