<!--
This file is the assumption ledger the PH-FT skill cites in its reports.
It is a snapshot of src/ft/ASSUMPTIONS.md in the same repo. If you edit
one, edit both. The content below the preamble is byte-identical to
src/ft/ASSUMPTIONS.md as of the snapshot date. Cite specific sections
(e.g. §3 row 8 for H100 BF16 peak) when writing recommendations.
-->
...
| 20 | Vendor coverage | Hard-coded to Modal / Lambda / Runpod (`Vendor` union + parallel `${vendor}_per_hr` fields). Adding Together / Fireworks / CoreWeave / AWS requires editing the type union AND every JSON row AND the refresh script. **Out of scope by design** — three vendors give us a defensible "cheapest of three" floor; exhaustive vendor coverage isn't the goal. **GPU SKU list within those three vendors is now auto-discovered** by `scripts/refresh-prices.ts` (`discoverGpuSkus`): the nightly refresh scans each vendor's scraped markdown for SKUs of the form `[Nx]<family> <N>GB` against an allowlist of known GPU families (H100/H200/B100/B200/A100/L4/L40S/MI300X/…) and unions them with the existing cached SKUs. New SKUs (e.g. B200 192GB the day Modal lists it) flow into `pricing.json` automatically with only the four scraped fields. They are inference-only until a human adds `bf16_tflops` etc., because `pickFtGpu` filters on `bf16_tflops > 0`. | Project decision | HIGH | N — scope-limited to three vendors; SKU coverage within those vendors is now auto-expanding. |
| 21 | Inference throughput formula (skill `calc.py`) | **Memory-bandwidth-bound decode**, per [injuly.in napkin-cost](https://injuly.in/blog/napkin-inference-cost/index.html). Each forward pass moves weights once + every active stream's KV cache from VRAM, then emits `concurrency` (batch `B`) tokens: `per_gpu_tok/s = B / ((weights_GB + kv_per_stream_GB × B) / mem_bandwidth)`. `B` is auto-picked as `min(VRAM-capacity, compute_intensity/2)` where `compute_intensity = bf16_flops / mem_bandwidth` (the roofline balance point — decode does ~`2B` ops per byte loaded). `gpus_needed = ceil(required_tok/s / per_gpu_tok/s)`, and self-host cost scales by it. NB: the **web app** (`src/engine.ts`) still uses the older `~120 tok/s per 8B active × batchingMultiplier` heuristic; the two are being reconciled — the skill is now the bandwidth-physics implementation. | B200 napkin-math anchor (32B, 200k ctx → ~6 users, ~40 tok/s/user); vLLM/TGI KV-cache slot math | MED | **Y** — reconcile web app `engine.ts` to the same bandwidth model. |
